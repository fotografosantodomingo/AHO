...
8. **Controlled seed data is allowed; never misrepresenting photos; everything removable.** (Policy CHANGED 2026-06-07 by PO — reversal of the prior absolute no-fake-data rule; see `DECISIONS.md` + `docs/SEED_LOG.md`.) Cold-start **seed listings + seed agents are permitted in production** to populate the marketplace while real inventory grows — under strict conditions: (a) every seed row tagged **`data_origin='seed'`** on `organizations`/`profiles`/`properties` so it is queryable + removable via `scripts/remove-seed.ts` (one-way immutability trigger blocks laundering seed→real); (b) **genuinely distinct, realistic, bilingual EN/ES content** — no templated/spun listings (Google's scaled-content policy flags those); (c) **NO photos that misrepresent** — seed listings are text-only with the neutral placeholder; **never** stock, AI-generated, or scraped photos in property slots (reverse-image-searchable = irreversible trust loss); (d) seeds are retired when real inventory replaces them. Real data is `data_origin='real'` (default). The photo prohibition + the distinct-content + removability requirements are non-negotiable. Test fixtures stay under `tests/` with `aho-test-org-`/`aho-fixture-` slugs (filtered from all public surfaces); seed data uses real-looking slugs (so it indexes) and is distinguished only by `data_origin`.
