...
- **MCP server (21st.dev Magic).** `.mcp.json` registers the `21st-magic` server with `API_KEY` sourced from `${TWENTY_FIRST_DEV_API_KEY}`. Because there's no direnv, the key must be in the parent shell when launching Claude Code. Workflow: `set -a && source .env.local && set +a && claude` (then approve the project-scoped MCP server on first prompt). Restart Claude Code after editing `.mcp.json` for changes to take effect.
