...
/**
 * Fetches the Inter Bold (700) woff2 from /public/fonts/ at OG-render
 ...
 *   - `import 'inter-700.woff2'` in the Edge runtime relies on bundler
 *     binary-import support that Next 15 + Wrangler treat inconsistently.
 *     A runtime fetch from `/fonts/inter-700.woff2` always works because
 *     it goes through the same static-asset path as our images.
 *   - Cloudflare Pages caches the woff2 with the same long-TTL header it
...
export async function loadInterBold(): Promise<ArrayBuffer | null> {
  ...
  try {
    ...
    const res = await fetch(`${origin.replace(/\/$/, '')}/fonts/inter-700.woff2`);
    if (!res.ok) {
      cached = null;
      return null;
    }
