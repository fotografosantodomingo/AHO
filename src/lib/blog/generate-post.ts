...
function buildPrompt(topic: BlogTopic): { system: string; user: string } {
  ...
  const user = [
    ...
    '',
    '6. At least one responsive <img> template anywhere in the body using srcset + sizes attributes AND explicit width + height (so the browser reserves layout space before the image loads — protects the CLS score). Use this EXACT markup (the cron stamps real URLs at publish):',
    '   <img src="{HERO_IMG}" srcset="{HERO_IMG_400} 400w, {HERO_IMG_800} 800w, {HERO_IMG_1200} 1200w" sizes="(max-width: 800px) 100vw, 800px" width="1200" height="630" loading="lazy" decoding="async" alt="...">',
    '   Choose a descriptive alt text that matches the visual you would expect to accompany the article. Do NOT inline a real photo URL — leave the placeholders intact. The width / height attributes MUST be present and MUST be `1200` and `630` (matches the hero image dimensions); their omission triggers a CLS regression.',
    '',
