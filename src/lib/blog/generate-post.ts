...
function buildPrompt(topic: BlogTopic): { system: string; user: string } {
  ...
  const user = [
    ...
    '',
    '6. At least one responsive <img> template anywhere in the body using srcset + sizes attributes. Use these exact placeholder URLs (the cron stamps real URLs at publish):',
    '   <img src="{HERO_IMG}" srcset="{HERO_IMG_400} 400w, {HERO_IMG_800} 800w, {HERO_IMG_1200} 1200w" sizes="(max-width: 800px) 100vw, 800px" alt="...">',
    '   Choose a descriptive alt text that matches the visual you would expect to accompany the article. Do NOT inline a real photo URL — leave the placeholders intact.',
    '',
