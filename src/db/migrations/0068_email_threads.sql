...
--
-- Two tables:
--   ai_email_threads   — one row per RFC-5322 email thread; pinned to
...
--   ai_email_messages  — one row per individual email (inbound or
...
--
-- Cross-reference: every ai_email_messages row points at exactly one
...
-- channel-specific blob; ai_email_messages stores the full headers +
...
create table public.ai_email_threads (
  id uuid primary key default gen_random_uuid(),
  -- Parent conversation. Many ai_email_threads can theoretically point
  ...
  -- ai_email_messages — see idx_email_msg_message_id).
...
create unique index idx_email_thread_root on public.ai_email_threads(root_message_id);
...
create index idx_email_thread_conv on public.ai_email_threads(conversation_id);

-- ─── ai_email_messages ───────────────────────────────────────────────

create table public.ai_email_messages (
  id uuid primary key default gen_random_uuid(),
  thread_id uuid not null references public.ai_email_threads(id) on delete cascade,
  -- 1:1 link to the channel-agnostic turn row. Every inbound email
  -- creates one ai_conversation_messages (role='user', channel=
  -- 'email') AND one ai_email_messages row. Outbound email reply does
...
create index idx_email_msg_message_id on public.ai_email_messages(message_id);
--   thread timeline (the dashboard renders this).
create index idx_email_msg_thread on public.ai_email_messages(thread_id, created_at);
--   conversation-message FK reverse lookup (rendering email-channel
--   metadata in the unified conversation timeline).
create index idx_email_msg_conv_msg on public.ai_email_messages(conversation_message_id);

-- ─── RLS ──────────────────────────────────────────────────────────

alter table public.ai_email_threads enable row level security;
...
create policy "org_members_read_ai_email_threads" on public.ai_email_threads
  ...
  using (
    exists (
      ...
      where c.id = ai_email_threads.conversation_id
        and om.user_id = auth.uid()
    )
  );

create policy "org_members_read_ai_email_messages" on public.ai_email_messages
  for select to authenticated
  using (
    exists (
      select 1
      from public.ai_email_threads t
      join public.ai_conversations c on c.id = t.conversation_id
      join public.organization_members om on om.org_id = c.org_id
      where t.id = ai_email_messages.thread_id
        and om.user_id = auth.uid()
    )
  );
...
revoke insert, update, delete on public.ai_email_threads from anon, authenticated;
revoke insert, update, delete on public.ai_email_messages from anon, authenticated;

comment on table public.ai_email_threads is
...
comment on table public.ai_email_messages is
  'Per-email rows within an ai_email_threads thread. Carries the RFC-5322 routing headers + DKIM/SPF/DMARC verification captured at ingest by workers/inbound-email. 1:1 with ai_conversation_messages for email-channel turns.';
