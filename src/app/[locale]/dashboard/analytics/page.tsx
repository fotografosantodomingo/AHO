...
import { AiAnalyticsTab } from '@/components/analytics/ai-tab';
import Link from 'next/link';

export const runtime = 'edge';
export const dynamic = 'force-dynamic';

type TabKey = 'performance' | 'ai';

/**
 * /{locale}/dashboard/analytics — agent performance dashboard.
 *
 * Two tabs (selected via `?tab=`):
 *   - performance (default) — property events + top listings + activity feed
 *   - ai — conversion stack metrics from `ai_daily_stats` (Phase 2 of
 *     `docs/AI_CONVERSION_PLAN.md`)
 *
 * Both read through the user-context Supabase client; RLS gates results
 * to the agent's own org. Empty states are honest emptiness per
...
export default async function AnalyticsPage({
  ...
  const tab: TabKey = tabParam === 'ai' ? 'ai' : 'performance';
  ...
  const baseHref = localePath(typedLocale, '/dashboard/analytics');
  ...
  return (
    <main className="space-y-8">
        ...
        {tabs.map((tabItem) => {
          ...
          return (
            <Link
              ...
              className={`-mb-px inline-flex items-center border-b-2 px-3 py-2 text-sm font-medium transition ${
                active
                  ? 'border-action text-action dark:text-action-dark'
                  : 'border-transparent text-helper hover:text-default'
              }`}
            >
              {tabItem.label}
            </Link>
      ...
      {tab === 'ai' ? (
        <AiAnalyticsTab orgId={orgId} locale={typedLocale} />
      ) : (
        <PerformanceTab orgId={orgId} locale={typedLocale} />
      )}
    </main>
  );
}
...
async function PerformanceTab({
  ...
  const [analytics7d, analytics30d, topListings, activity] = await Promise.all([
    ...
    getTopListingsByEngagement(supabase, orgId, since30d, locale, 10),
    getRecentActivity(supabase, orgId, locale, 20),
  ]);

  const hasAnyData = analytics30d.total > 0;
  const browseHref = localePath(locale, '/dashboard/properties');
