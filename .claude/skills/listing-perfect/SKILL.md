...
- **`force-dynamic` without Cache-Control** = ~1.1s server response on every request. Always pair `force-dynamic` with explicit Cache-Control via `next.config.ts:headers()` for high-traffic public pages.
