...
| 4 | **Admin SQL view + tiny dashboard** for `ai_generation_log` rollups (daily cost, per-market cost, per-audit cost distribution) so the unit-economics number is visible without a one-off psql session. | Phase 5.5 follow-on: now that logging is in place, we need the read surface. | S | Me |
| 5 | **Wire `ai_generation_log` writes into the `importFromUrl` Sonnet call** — today only the 3 drafter calls log; the scrape adds ~30-50% of audit cost we're not seeing. | Without it, the per-audit cost number in the dashboard is materially understated. | S | Me |
...
- Wire `ai_generation_log` writes into the `importFromUrl` Sonnet call too (today only the 3 drafter calls log; the scrape adds another ~30-50% of audit cost we're not seeing)
