...
### Batch 2 (2026-06-07): one agent + one listing per country (100 countries)

- **100 solo agents**, one per country (the 100 most-populous, **DR excluded**), each its own agency (`organizations.slug = seed-agent-<iso2>`, `public_slug = <name>-<iso2>`) so each has a public profile page. Country-appropriate names/bios.
- **100 listings**, one per country, in that country's capital. Distinct bilingual content, local-currency prices.
- Generated by `scripts/seed-agents-per-country.ts` (idempotent per country — skips a country whose `seed-agent-<iso2>` org exists). Seed-agent emails: `agent-<iso2>@aho-seed.test`.

Combined seed total after both batches: ~200 listings + ~106 agents + ~103 agencies — all `data_origin='seed'`.
