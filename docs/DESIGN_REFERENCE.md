# AHO visual design reference

This is the **canonical visual language for AHO**. Captured 2026-04-29 from the `getdesign` npm package (`getdesign@0.6.10`, template `hashicorp.md`). Source landing page: <https://getdesign.md/hashicorp/design-md>.

## How this applies to AHO

- **Use as the parent design system.** Color tokens, type scale, spacing, radii, shadow philosophy, button/input/card patterns, and the dense-headings / relaxed-body rhythm are adopted directly.
- **Substitute the brand font.** AHO does NOT have a license for HashiCorp Sans. Replace the brand font with **Inter** (variable, weights 500–700) — it preserves the dense, infrastructural feel and is OFL-licensed. Keep the system-ui stack for body text.
- **Drop the multi-product color system.** AHO is a single product, not a portfolio. Use the parent black-and-white palette + Action Blue (`#1060ff` on dark, `#2264d6` on light) as the only accent. No rainbow product tokens.
- **Preserve the dual-mode (light + dark hero).** The "build in light, deploy in dark" duality maps to AHO as: search/listings/dashboards in light, marketing hero + pricing + onboarding can use the dark `#15181e` panels for contrast.
- **Tailwind translation.** Map the spec's tokens to Tailwind v4 CSS variables (`@theme`) so component code reads as `bg-surface-dark`, `text-helper`, `rounded-card`, not raw hex. Existing `zinc-*` utilities are a holding pattern; migrate as components are touched.

The original HashiCorp spec follows verbatim below.
